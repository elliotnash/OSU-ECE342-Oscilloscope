...
fn serialize_12bit_data<S>(data: &Vec<u16>, serializer: S) -> Result<S::Ok, S::Error>
where
    S: Serializer,
{
...
pub struct FrameData {
    #[serde(serialize_with = "serialize_12bit_data", deserialize_with = "deserialize_12bit_data")]
    pub data: Vec<u16>,
    pub timescale: f32,
...
mod tests {
    ...
    fn heartbeat_serialization() {
        ...
        let deserialized = postcard::from_bytes_cobs::<Message>(&mut bytes).expect("Deserialization failed");
    ...
    fn frame_data_packing_even() {
        ...
        let deserialized = postcard::from_bytes_cobs::<FrameData>(&mut bytes).expect("Deserialization failed");
